    ...
    export class QQN {
        ...
        oracleDirection(g, d_sd) {
            ...
            if (!isFiniteVec(d_lbfgs)) {
                console.warn('[QQN] oracle produced non-finite direction; ' +
                    'falling back to steepest descent', {d_lbfgs, g});
                d_lbfgs = d_sd.slice();
            }
            return d_lbfgs;
        }
        ...
        step(obj) {
            ...
            if (p.verbose) {
                console.debug('[QQN] step', {
                    n: this.stepCount, chosenT,
                    f0, f1: obj.value(to[0], to[1]),
                    probes: probes.length, histLen: this.lbfgs.s.length,
                });
            }
